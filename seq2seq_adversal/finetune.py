...
class PrefixSummarizationModule(PrefixTransformer):
    ...
    def _step(self, batch: dict) -> Tuple:
        ...
        outputs, domain_value = self(src_ids, oracle_ids=oracle_ids, attention_mask=src_mask, decoder_input_ids=decoder_input_ids, use_cache=False,
                       use_prefix=True)


        #print(domain_value.shape)
        #print(batch["domain_labels"])

        lm_logits = outputs[0]


        loss_id = torch.nonzero(batch["domain_labels"])

        loss_id  =loss_id.squeeze(1)
        #print(torch.nonzero(batch["domain_labels"]))
        #print(batch["domain_labels"])
        '''
        ...
        print(loss_id)
        '''
        #lm_logits = lm_logits[loss_id]
        lm_logits = torch.index_select(lm_logits, 0, loss_id)
        #tgt_ids = lm_logits[loss_id]
        tgt_ids = torch.index_select(tgt_ids, 0, loss_id)
        ...
        domain_loss_fc = torch.nn.CrossEntropyLoss()
        domain_loss = domain_loss_fc(domain_value, batch["domain_labels"])

        loss  = 0.01 * domain_loss + loss

        #if batch["domain_labels"] == 
    ...
    def _generative_step(self, batch: dict) -> dict:
        ...
        # 此处为推理阶段一个重要过程, 需要和训练过程保持一致
        self.seq2seq_model = self.seq2seq_model.cuda()
        forzen_hiddenstates = self.seq2seq_model.model(batch["input_ids"].to(self.seq2seq_model.device), return_dict = True).last_hidden_state

        dist_list = []
        dist_list = cal_dist(self.model.train_domain_embeding, forzen_hiddenstates, bsz, dist_list)
        dist_list = cal_dist(self.model.taxi_domain_embeding, forzen_hiddenstates, bsz, dist_list)
        dist_list = cal_dist(self.model.hotel_domain_embedding, forzen_hiddenstates, bsz, dist_list)
        dist_list = cal_dist(self.model.attraction_domain_embedding, forzen_hiddenstates, bsz, dist_list)
        dist_list = cal_dist(self.model.restaurant_domain_embedding, forzen_hiddenstates, bsz, dist_list)
        ...
        for i in range(top20_indices.shape[0]):
            ...
            for j in range(top20_indices.shape[1]):
                ...
                input_token.append(self.model.all_domain_words[top20_indices[i, j]])
            input_token = " ".join(input_token)
            #print(input_token)

            input_token_list.append(input_token)

        input_ids_temp = self.model.forzen_gpt2_tokenizer(input_token_list, return_tensors="pt", padding=True, truncation=True)["input_ids"]
        forzen_decoder_input_ids = shift_tokens_right(input_ids_temp, self.model.pad_token_id)

        forzen_output = self.seq2seq_model(input_ids_temp.to(self.seq2seq_model.device), decoder_input_ids=forzen_decoder_input_ids.to(self.seq2seq_model.device), use_cache=True,
            return_dict=True)
        ...
        prefix_prompt, domain_value = self.model.get_prompt(bsz=bsz, h0_list = output_list_new, dst_ids= batch["oracle_ids"], sample_size=self.eval_beams)
        # print(prefix_prompt)
